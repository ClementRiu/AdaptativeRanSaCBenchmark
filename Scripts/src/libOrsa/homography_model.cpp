...
void HomographyModel::Fit(const std::vector<int> &indices, std::vector<Mat> *H, const double *weights_) const {
    if (4 > indices.size())
        return;
    const int n = static_cast<int>( indices.size());
    Eigen::MatrixXd A(n * 2, 9);
    for (int i = 0; i < n; ++i) {
        ...
        A(j, 1) = x1(1) * weight;
        A(j, 2) = weight;
        A(j, 6) = -x2(0) * x1(0) * weight;
        A(j, 7) = -x2(0) * x1(1) * weight;
        A(j, 8) = -x2(0) * weight;
        ...
        A(j, 4) = x1(1) * weight;
        A(j, 5) = weight;
        A(j, 6) = -x2(1) * x1(0) * weight;
        A(j, 7) = -x2(1) * x1(1) * weight;
        A(j, 8) = -x2(1) * weight;
    }
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(A, Eigen::ComputeFullV);
    const Eigen::Matrix<double, 9, 1> null_space = svd.matrixV().rightCols<1>();
    ...
    if (weights_ == nullptr && libNumerics::SVD::InvCond(M) < ICOND_MIN)
        return;
    Unnormalize(&M);
    if (M.det() < 0)
        M = -M;
    M /= M(2, 2);
    if (weights_ != nullptr || IsOrientationPreserving(indices, M))
        H->push_back(M);
}
