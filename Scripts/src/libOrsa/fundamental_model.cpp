...
//        x1 = N1_ * x1;
//        x2 = N2_ * x2;
//        (*A)(i, 0) = x1(0) * x2(0) * weight;  // 0 represents x coords,
//        (*A)(i, 1) = x1(0) * x2(1) * weight;  // 1 represents y coords.
//        (*A)(i, 2) = x1(0) * weight;
//        (*A)(i, 3) = x1(1) * x2(0) * weight;
//        (*A)(i, 4) = x1(1) * x2(1) * weight;
//        (*A)(i, 5) = x1(1) * weight;
//        (*A)(i, 6) = x2(0) * weight;
//        (*A)(i, 7) = x2(1) * weight;
//        (*A)(i, 8) = weight;
...
//    // Set up the homogeneous system Af = 0 from the equations x'T*F*x = 0.
//    Mat A(indices.size(), 9);
//    EpipolarEquation(indices, &A, weights_);
//
//    if (indices.size() < 8)
//        algo7pt(A, Fs, indices);
//    else
...
void FundamentalModel::Fit(const std::vector<int> &indices,
                           std::vector<Model> *Fs) const {
    // Set up the homogeneous system Af = 0 from the equations x'T*F*x = 0.
    Eigen::MatrixXd A(indices.size(), 9);
    EpipolarEquation(indices, &A);
...
/// \param Fs Output vector of found F matrices (up to 3)
/// \param indices The indices of points used to check coherency of F
    void FundamentalModel::algo7pt(const Eigen::MatrixXd &A, std::vector<Mat> *Fs,
                                   const std::vector<int> &indices) const {
        // Find two F matrices in the nullspace of A
        Mat F1(3, 3), F2(3, 3);
        Eigen::JacobiSVD<Eigen::MatrixXd> svd(A, Eigen::ComputeFullV);
        const Eigen::Matrix<double, 9, 1> null_space1 = svd.matrixV().array().col(7);
        const Eigen::Matrix<double, 9, 1> null_space2 = svd.matrixV().array().col(8);
        for (int ind = 0; ind < 9; ind++) {
            F1(ind) = null_space1(ind);
            F2(ind) = null_space2(ind);
        }
        F2 -= F1;
...
void FundamentalModel::algo8pt(const Eigen::MatrixXd &A, std::vector<Mat> *Fs) const {
    // Without rank constraint
    Eigen::JacobiSVD<Eigen::MatrixXd> svd(A, Eigen::ComputeFullV);
    const Eigen::Matrix<double, 9, 1> null_space = svd.matrixV().rightCols<1>();
    Eigen::Matrix<double, 3, 3> M;
    for (int ind = 0; ind < 9; ind++) {
        M(ind) = null_space(ind);
    }
    // Force the rank 2 constraint
    typedef Eigen::Matrix<double,3,3,Eigen::RowMajor> Mat;
    Eigen::JacobiSVD<Mat> svdResult(M,
                              Eigen::ComputeFullU|Eigen::ComputeFullV);
    Eigen::Vector<double,3> D = svdResult.singularValues();
    D(2)=0;
    M = svdResult.matrixU()*D.asDiagonal()*svdResult.matrixV().transpose();
    libNumerics::matrix<double> F2(3, 3);
    for (int ind = 0; ind < 9; ind++) {
        F2(ind) = M(ind);
    }
