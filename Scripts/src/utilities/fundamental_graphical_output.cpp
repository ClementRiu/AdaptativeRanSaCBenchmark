...
namespace utility {
    ...
    vector<double> epipolar_line(const matrix<double> &F, const Match &m) {
    ...
    void project_on_epi(double &x, double &y, const vector<double> &epi) {
    ...
    void fundamental_graphical_output(const Image<unsigned char> &image1,
        ...
        if (fileEpi) {
            ...
            // Draw small epipolar lines for inliers
            if (F && F->nrow()==3 && F->ncol()==3)
                for (it = vec_in.begin(); it != vec_in.end(); ++it) {
