...
inFolderGlob=${inFolderGlob:--1} #Datas will be written in this folder in a folder whose name is time of launch.
...
genEnd=${genEnd:--1}
genGenerateSIFT=${genGenerateSIFT:-0}
genForce=${genForce:-0}
...
#Generation of the dataset: jpg images
genCounter=${genBegin}
...
if [ "${inputTimeFolder}" = "-1" ];then
    ...
    do
        ...
        execArgs="${genArgs}"
        if [ "${genGenerateSIFT}" == "0" ];then
            execArgs="${execArgs} ${inMatches}"
        else
            execArgs="${execArgs} --generate-sift"
        fi
        execArgs="${execArgs} ${inImg1} ${inImg2}"
        if [ "${genModel}" == "2" ];then
...
#Generation of the dataset: png images
genCounter=${genBegin}
...
if [ "${inputTimeFolder}" = "-1" ];then
    ...
    do
        inFolder=${inFolderGlob}test${genCounter}/
        inMatches=${inFolder}orig_pts.txt
        inImg1=${inFolder}im1.png
        inImg2=${inFolder}im2.png
        inCalib=${inFolder}calib_matrices.txt
...
do
    ...
    do
        ...
        do
            ...
            ~/Documents/RANSAC-benchmark/Scripts/Build/experiments/general_experiment ${execArgs}
